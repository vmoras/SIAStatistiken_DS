...
public class AutoSIA {
    ...
    private static SimpleList<String> seenCourses = new SimpleList<>();
    ...
    public static void setCourses(DoubleList<Course> savedData){
        courses = savedData;
    }
    ...
    private static synchronized boolean courseSeen (String code){
        ...
        if (seenCourses.containsValue(code) != null){
            return true;
        }
    ...
    private void getCourseInfo(String code){
        ...
        SimpleList<WebElement> numGroups = new SimpleList<>(numGroupsList);
        ...
        if (numGroups.getHead().getData().getText().equals("Contenido de la asignatura")){
            numGroups.removeFirst();
        }
        ...
        if (course == null){
            // Create a node with the course's code n' name, and a queue for the groups
            String name = fullName.substring(0, fullName.indexOf("("));
            course = new Course(code, name);
            Queue<Group> data = new Queue<>();
        ...
        for (int i = 0; i < groupsInfo.length; i++){
            ...
            if (satisfyCondition(spots)){
                ...
                if (groupInfo == null) {
                    ...
                    course.getGroups().enqueue(newGroup);
                }

                // If the teachers name isnt known, update it
        ...
        // Check if any of the groups doesn't satisfy the condition anymore (have 0 spots), if so delete it
    ...
    private static synchronized Course isCourseSaved(String code){
        ...
        return courses.containsValue(code);
    }
    ...
    private static synchronized Group isGroupSaved(Course course, String numGroup){
        ...
        return course.getGroups().containsValue(numGroup);
    }
    ...
    // TODO: add a predicate so the remove function could accept satisfyCondition as argument. Check:
    //  ArrayList<String> a = new ArrayList<>();
    //        a.removeIf()
    /*private void checkQueue(Queue<Group> queue){
        Group group;
        while((group = queue.containsValue(0)) != null){
            queue.remove(group);
        }
    }*/
